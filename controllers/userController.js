...
const addToWishlist = asyncHandler(async (req, res) => {
  ...
  if (user && product) {
    ...
    res
      .status(400)
      .json({ message: "Impossible d'obtenir les informations utilisateur et/ou produit." });
  }
});
...
const removeFromWishlist = asyncHandler(async (req, res) => {
  ...
  if (user && product) {
    try {
      ...
      product.analytics.likes = product.analytics.likes.filter((id) => id != req.user.id);
      await user.save();
      await product.save();
      res.status(200).json({status: "success"})
    } catch (err) {
...
module.exports = {getMyUserInfo, updateMyUserInfo, findUserInfo, addToWishlist, removeFromWishlist};
